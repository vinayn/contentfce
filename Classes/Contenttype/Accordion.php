...
class Accordion {
		...
		public function renderAccordion($PageDataholderObj){
					...
					foreach ( $data["lDEF"] as $fieldscontainer => $fieldscontainervalue ) {
						...
						if(isset($fieldscontainervalue['el']) && is_array($fieldscontainervalue['el']))
						{
							...
							foreach ( $fieldscontainervalue['el'] as $eachcolumn => $eachcolumnData ) {
								...
								$tabtitle = $eachcolumnData['panel']['el']['title']['vDEF'];
								$eachcolumn = $eachcolumn -1;	
								$resChildren = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_content', 'pid=' . $PageDataholderObj->pageid . ' AND sys_language_uid=' . $PageDataholderObj->sys_language .BackendUtility::deleteClause('tt_content') . BackendUtility::versioningPlaceholderClause('tt_content').' and tx_flux_parent = '.$PageDataholderObj->cRow['uid']."  and tx_flux_column = 'content_".$eachcolumnData['panel']['el']['id']['vDEF']."'" , '', 'sorting');
									...
									{
											$contentColdata .= PHP_EOL."\t\t\t".'<section>'."\t\t\t".PHP_EOL."\t\t\t". '<title>'.$tabtitle.PHP_EOL."\t\t\t".'</title>'.$eachcontentCol .PHP_EOL."\t\t\t".'</section>'.PHP_EOL;
